...
class ConversationLoop:
  ...
  Natural language:
    ...
    def _detect_chapter_write(self, response, messages: list) -> int | None:
        ...
        # Detect chapter prose: has paragraphs + dialogue + sentence density
        # Skip if it looks like outline (many numbered chapters), planning doc, or conversation
        has_paragraphs = text.count("\n\n") >= 2
        has_dialogue_quotes = "“" in text or '"' in text or "「" in text
        has_sentence_density = text.count("。") >= 10
        looks_like_prose = has_paragraphs + has_dialogue_quotes + has_sentence_density >= 2

        # Check for outline patterns (generated by agent despite routing instructions)
        outline_count = sum(1 for s in ["第1章", "第2章", "第3章", "卷级规划"] if s in text[:800])
        if not looks_like_prose or outline_count >= 3:
            return None
