...
class AIAgent:
    ...
    def build_system_prompt(self) -> str:
        ...
        parts.append(
            ...
            "## 工作原则\n"
            "## 对话模式创作流程（严格遵守）\n"
            "- 一次只做一件事。完成当前步骤后必须停下来等待用户确认，再进入下一步。绝对禁止连续执行多个步骤\n"
            "- 标准流程：1.大纲 → 2.角色 → 3.世界观 → 4.第一章。每步完成后问用户'下一步做什么？'，不替用户决定\n"
            "- 用户说'继续'时，判断当前缺失什么——缺大纲则先提议大纲，有纲无角则提议角色，齐全了才提议写第一章\n"
            "- 指令模糊时（如'帮我写'）必须追问：写哪章？什么内容？多少字？\n"
            "- 不确定的情节走向、角色决策、风格选择主动提出2-3个选项\n\n"
            "## 输出路由（重要）\n"
