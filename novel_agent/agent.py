...
class AIAgent:
    ...
    def build_system_prompt(self) -> str:
        ...
        parts.append(
            ...
            "## 内容预览与保存流程\n"
            "所有需要保存的内容（章节/大纲/设定）必须走两步：\n"
            "\n"
            "1. 先输出内容（流式展示给用户看）\n"
            "2. 再调用预览工具让用户确认：\n"
            "   - 章节 → preview_chapter(chapter_number=N, title='...')\n"
            "   - 大纲 → preview_outline(content='...')\n"
            "   - 角色/世界观 → preview_setting(setting_type='character'|'world'|'plot'|'style', name='...', description='...', content='...')\n"
            "\n"
            "如果用户选择保存，系统会自动持久化。如果用户丢弃，内容不会保存。\n"
            "永远不要跳过预览工具直接操作——用户必须看到并确认后才保存。\n\n"
