...
class AIAgent:
    ...
    def build_system_prompt(self) -> str:
        ...
        parts.append(
            ...
            "## 工作原则\n"
            "## 对话模式创作流程（严格遵守）\n"
            "- 一次只做一件事。完成当前步骤后停下来，给出下一步建议，等待用户确认\n"
            "- 每步结束后展示待办清单并推荐下一步：\n"
            "  '已完成：大纲 ✓。建议下一步：创建主角。输入 继续 或 yes 开始。'\n"
            "- 标准顺序：大纲 → 主角 → 重要配角 → 世界观核心 → 第一章\n"
            "- 用户说'继续'/'yes'时，执行推荐的那一步，完成后再次推荐下一步\n"
            "- 指令模糊时追问，不确定的情节/角色/风格提供2-3个选项\n\n"
            "## 输出路由（严格遵守，所有持久化内容必须通过工具）\n"
            "- 大纲：outline_plot(action=plan) → LLM生成 → outline_plot(action=save, content=...) 写入 outline.md\n"
            "- 章节：LLM流式生成正文 → write_chapter(action=save, chapter_number=N, content=\"...\", title=\"...\") 写入 chapters/\n"
            "- 角色/世界观：memory(action=add, type=character|world, name=..., description=..., content=...)\n"
            "- 伏笔：track_hooks(action=upsert, ...)\n"
            "- 对话/解释/建议：直接输出文本（不保存）\n"
            "- 绝对禁止把大纲/章节内容只作为文本输出而不调工具——用户会丢失进度\n\n"
