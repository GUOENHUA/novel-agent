...
<type>
    ...
    对话风格、章末处理方式。全局只保存一份，写作前设定，不需要按章节更新。风格记忆帮助你保持整本书的写作风格一致。
    记录成功的验证（不仅是修正），以防风格漂移。
    </description>
    <when_to_save>
    用户给出风格反馈时、确认某种写法"对了"时、确定风格偏好时。
    用户提供参考文本时——设置 frontmatter active_until_chapter: N，
    到期后系统自动移除，不再注入上下文。
    </when_to_save>
    <how_to_use>
    ...
    用户: 我给你一段参考文字，前20章照着这个感觉写 (粘贴了参考文本)
    助手: [保存风格记忆: frontmatter 加 active_until_chapter: 20。正文存完整参考段落。
    20章后系统自动跳过，不再注入上下文。格式: 约束用一行概括，参考文本存正文。]
    </examples>
</type>
</types>"""
